...
public class PercentViewJeju extends RelativeLayout implements PercentView {
    TextView Jeju, Seogwipo;
    String[] subLocality = {"서귀포시", "제주시"};

    public PercentViewJeju(Context context, View view) {
        super(context);
        this.Jeju = view.findViewById(R.id.percent_Jeju_jeju);
        this.Seogwipo = view.findViewById(R.id.percent_Jeju_seogwipo);
    }
    ...
    public void setPercentage() {
        ...
        ownDocRef = db.collection("RealTimeWeather").document("제주특별자치도").collection(subLocality[0]).document("total");
        ownDocRef.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                ...
                setBackground(Seogwipo, ratio);
                Seogwipo.setText(message);
            }
        });
        ownDocRef = db.collection("RealTimeWeather").document("제주특별자치도").collection(subLocality[1]).document("total");
        ownDocRef.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            @Override
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                RealTimeVote total = documentSnapshot.toObject(RealTimeVote.class);
                ...
                String message = Integer.toString(percent);
                message = message.concat("%");
                setBackground(Jeju, ratio);
                Jeju.setText(message);
            }
        });
    }
}
